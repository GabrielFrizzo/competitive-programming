...
int main(int argc, char const *argv[]) {
	...
	for(int i = 0; i < N; ++i) {
		...
		if (x1 != x2 && y1 != y2 && (abs(x2-x1) != abs(y2-y1))) continue;
	...
	for (line l : lines) {
		if (l.isHoriz()) {
			...
			int dirx = l.p1.x < l.p2.x ? 1 : -1;
			int diry = l.p1.y < l.p2.y ? 1 : -1;
			for(int x = l.p1.x, y = l.p1.y; ((x <= l.p2.x && dirx == 1) || (x >= l.p2.x && dirx == -1))
																	 && ((y <= l.p2.y && diry == 1) || (y >= l.p2.y && diry == -1)); x+= dirx, y += diry) {
				m[y][x]++;
			}
		}
	}
